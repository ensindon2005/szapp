...
def explore(filename):
    ...
    if extension in excel:
       ...
       try:
            df=pd.read_csv(path_file, sep="\s+|;|:|,", encoding=encodes[0],index_col=False)
            
       except:
            df=pd.read_csv(path_file,sep="\s+|;|:|,",encoding=encodes[1],index_col=False)    
    ...
    return render_template('explore.html', dtypes=safter,headers=headers,df=df,filename=filename,before=before)
